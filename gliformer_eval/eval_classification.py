#!/usr/bin/env python3
"""Evaluate GLiFormer on the zero-shot classification suite used by GLiClass.

This script mirrors ``GLiClass/test_gliclass.py`` while using GLiFormer's native
...
BENCHMARKS = (
    ...
    DatasetSpec(
        ...
        text_column="text",
        label_column="label",
        # MTEB exposes numeric class names; preserve the original sentiment ID order.
        classes=("negative", "neutral", "positive"),
    ),
    DatasetSpec("SetFit/ag_news", label_column="label_text"),
...
def create_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate GLiFormer on the GLiClass zero-shot classification suite.")
    parser.add_argument("--seed", type=int, default=42, help="Random sampling seed (default: 42).")
    parser.add_argument("--model", required=True, help="Local GLiFormer checkpoint or Hugging Face model ID.")
    parser.add_argument(
        "--api-key",
        "--api_key",
        "--token",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="GLiFormer inference batch size.",
    )
    parser.add_argument(
        "--threshold",
        type=float,
...
def prepare_dataset(
    ...
    *,
    ...
    """Normalize a loaded Hugging Face dataset for GLiFormer inference."""
    ...
    if max_samples is not None and max_samples < len(raw_texts):
        indices = random.Random(seed).sample(range(len(raw_texts)), max_samples)
        raw_texts = [raw_texts[index] for index in indices]
        raw_labels = [raw_labels[index] for index in indices]
...
def load_benchmark(
    ...
    *,
    ...
    if spec.name == "AmazonScience/massive":
        # The main branch needs a custom loader, and the Parquet branch has no
        # config metadata. Select the locale explicitly and retain ClassLabels.
        base = f"hf://datasets/{spec.name}@refs/convert/parquet/{spec.config}"
        return load_dataset("parquet", data_files={
            split: f"{base}/{split}/*.parquet"
            for split in ("train", "validation", "test")
        }, **kwargs)
    if spec.name == "PolyAI/banking77":
        # Read the same upstream CSVs as banking77.py without executing its
        # custom loader. Labels are already their original intent names.
        ...
        return load_dataset("csv", data_files={
            split: f"{base}/{split}.csv" for split in ("train", "test")
        }, column_names=["text", "label"], skiprows=1, skipinitialspace=True,
...
def _prediction_label(result: Any) -> str | None:
    """Extract the top class name across supported GLiFormer output shapes."""
...
def predict_labels(
    ...
    *,
    ...
    """Run native single-label GLiFormer classification in bounded batches."""
    ...
    for start in range(0, len(texts), batch_size):
        ...
        if len(batch_results) != len(text_batch):
            raise RuntimeError(
                f"GLiFormer returned {len(batch_results)} outputs for {len(text_batch)} classification inputs"
            )
