#!/usr/bin/env python3
"""Evaluate a GLiFormer checkpoint on the ``data/NER`` benchmarks.

The benchmark files contain character-level entity spans, and GLiFormer returns
...
def create_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate GLiFormer on data/NER datasets and report per-dataset F1.")
...
LABEL_ALIASES = {"organisation": "organization", "misc": "else"}


def canonical_label(label: str) -> str:
    """Unify benchmark spelling aliases on both sides of strict NER scoring."""
...
def load_dataset(dataset_dir: Path, split: str) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    records = [{**row, "entities": [
        {**entity, "type": canonical_label(entity["type"])} for entity in row["entities"]
    ]} for row in records]
    return records, list(dict.fromkeys(canonical_label(label) for label in labels))
...
def _gold_entity_key(entity: dict[str, Any]) -> tuple[int, int, str]:
    ...
    return int(start), int(end), canonical_label(str(label))
...
def evaluate_dataset(
    ...
    *,
    ...
    """Run GLiFormer inference and score one loaded dataset."""
    ...
    for batch in _batched(records, batch_size):
        ...
        if len(predictions) != len(batch):
            raise RuntimeError(
                f"GLiFormer returned an unexpected number of prediction lists: {len(predictions)} for {len(batch)} inputs"
            )
...
def load_model(model_id: str, device: str) -> Any:
    from gliformer import GLiFormer  # noqa: PLC0415 - importing torch is intentionally deferred
...
def save_report(
    ...
    *,
    ...
    payload = {
        ...
        "evaluation_protocol_version": 2, "label_aliases": LABEL_ALIASES,
        "macro_f1": _mean_f1(results),
...
def main() -> None:
    ...
    if args.output is not None:
        save_report(
            ...
            model_id=args.model,
            split=args.split,
            threshold=args.threshold, seed=args.seed, max_samples=args.max_samples,
        )
