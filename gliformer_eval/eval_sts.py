#!/usr/bin/env python3
"""Evaluate GLiFormer sentence embeddings on the STS Benchmark.

The ``sentence-transformers/stsb`` dataset contains ``sentence1``,
``sentence2``, and a human similarity ``score`` normalized to ``[0, 1]``.
This evaluator embeds both sentences with GLiFormer, calculates pairwise model
...
def create_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate GLiFormer embeddings on sentence-transformers/stsb.")
...
def load_model(
    ...
    *,
    ...
    """Load an embedding-capable GLiFormer checkpoint."""
    ...
    if getattr(model.config, "embedding_config", None) is None:
        raise ValueError(f"GLiFormer checkpoint {model_id!r} does not enable an embedding head")
    model.to(device)
    model.eval()
    return model
...
def embed_sentence_pairs(
    ...
    *,
    ...
    if not isinstance(embeddings, torch.Tensor) or embeddings.ndim != 2:
        raise RuntimeError(
            "GLiFormer embed_text() must return a rank-2 torch.Tensor, got "
            f"{type(embeddings).__name__} with shape {getattr(embeddings, 'shape', None)}"
        )
    if embeddings.shape[0] != sentence_count * 2:
        raise RuntimeError(
            f"GLiFormer returned {embeddings.shape[0]} embeddings for {sentence_count * 2} input sentences"
        )
...
def compute_similarities(
    ...
    """Calculate pairwise similarity using GLiFormer embedding-head semantics."""
    import torch  # noqa: PLC0415 - used only during model inference
    ...
    if not np.isfinite(prediction_array).all():
        raise ValueError("GLiFormer produced non-finite STS similarity predictions")
...
def print_report(result: CorrelationResult) -> None:
    print()
    print("GLiFormer STS evaluation")
...
def save_report(output_path: Path, result: CorrelationResult, *, model_id: str, seed: int = 42, max_samples: int | None = None) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    payload = {"model": model_id, "seed": seed, "max_samples": max_samples, **asdict(result)}
...
def main() -> None:
    ...
    if args.max_samples is not None and args.max_samples < len(gold_scores):
        indices = random.Random(args.seed).sample(range(len(gold_scores)), args.max_samples)
        sentences1 = [sentences1[index] for index in indices]
        sentences2 = [sentences2[index] for index in indices]
        gold_scores = [gold_scores[index] for index in indices]
    ...
    if args.output is not None:
        save_report(args.output, result, model_id=args.model, seed=args.seed, max_samples=args.max_samples)
