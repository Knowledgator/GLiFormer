"""GLiFormer — main user-facing class for multi-task information extraction."""
...
from .config import (
    ...
    GLIFORMER_MODEL_TYPE_TO_VARIANT,
    GLIFORMER_VARIANT_TO_CONFIG_CLASS,
    resolve_gliformer_config_class,
)
from .model import (
    GLiFormerAudioModel,
...
from .processing.processor import GLiFormerProcessor, resolve_gliformer_processor_class
from .processing.pdf import GLiFormerPDFProcessor
...
class BaseGLiFormer(BaseGLiNER):
    ...
    Example::
        ...
        model = GLiFormer.from_pretrained("knowledgator/gliformer-base")
    ...
    config_class = GLiFormerConfig
    model_class = None
    data_processor_class = GLiFormerProcessor
    data_collator_class = GLiFormerDataCollator
    decoder_class = GLiFormerDecoder
    ...
    def gradient_checkpointing_enable(self, gradient_checkpointing_kwargs=None):
        """Enable checkpointing on the transformer backbones wrapped by GLiFormer.

        Hugging Face's Trainer calls this method on its top-level model. GLiFormer
    ...
    def _create_model(self, config, backbone_from_pretrained, cache_dir, **kwargs):
        model_cls = self.model_class or resolve_gliformer_model_class(config)
    ...
    def _supported_base_loader_kwargs(loader, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if accepts_kwargs:
            ...
            # model kwargs, but only pass GLiFormer dispatcher options when the
    ...
    @classmethod
    def _load_config(cls, config_file: Path, **config_overrides) -> GLiFormerConfig:
        """Load GLiFormer config, dispatching factory loads by saved model_type."""
        ...
        if cls.config_class in specific_config_classes:
            ...
            config_cls = resolve_gliformer_config_class(config_dict)

        model_type = config_dict.pop("model_type", None)
        if model_type in GLIFORMER_MODEL_TYPE_TO_VARIANT:
            config_dict["model_variant"] = GLIFORMER_MODEL_TYPE_TO_VARIANT[model_type]
    ...
    def _get_special_tokens(self):
        ...
        if self.config.structuring_config is not None:
            ...
            if getattr(self.config.structuring_config, "multi_level", False):
                tokens.extend(
                    [
    ...
    def inference(
        ...
        **kwargs,
        ...
        Args:
            ...
            objectness_threshold: Optional relation/structuring anchor
                objectness threshold. When omitted, the decoder reuses
                ``threshold``.
    ...
    def parse_pdf(
        ...
        **kwargs,
        ...
        """Run GLiFormer layout/text inference over PDF pages.
        ...
        pdf_processor = GLiFormerPDFProcessor()
    ...
    def structure(
        ...
        **kwargs,
        ...
        task_results = results.get(task_name, [{} for _ in text_batch])
    ...
    def inference_from_schema(
        ...
        **kwargs,
        ...
        Args:
            texts: Input text(s).
            schema: A :class:`GLiFormerSchema` instance.
    ...
    def train_head_only_parameters(self) -> Dict[str, int]:
        ...
        if not hasattr(self, "model") or not hasattr(self.model, "heads"):
            raise ValueError("Head-only training requires an initialized GLiFormer model with task heads.")
    ...
    def train_model(
        ...
        **training_kwargs,
        ...
        Example::
            ...
            model = GLiFormer.load_from_config(config, output_dir="./output")
        ...
        trainer_parameters = inspect.signature(
            GLiFormerTrainer.__mro__[1].__init__
        ).parameters
        ...
        trainer = GLiFormerTrainer(**trainer_kwargs)
...
class GLiFormerText(BaseGLiFormer):
    """User-facing text GLiFormer wrapper."""

    config_class = GLiFormerTextConfig
    model_class = GLiFormerTextModel
    data_collator_class = GLiFormerTextDataCollator


class GLiFormerVision(BaseGLiFormer):
    """User-facing vision GLiFormer wrapper."""

    config_class = GLiFormerVisionConfig
    model_class = GLiFormerVisionModel
    data_collator_class = GLiFormerVisionDataCollator


class GLiFormerAudio(BaseGLiFormer):
    """User-facing audio GLiFormer wrapper."""

    config_class = GLiFormerAudioConfig
    model_class = GLiFormerAudioModel
    data_collator_class = GLiFormerAudioDataCollator


class GLiFormerLayout(BaseGLiFormer):
    """User-facing document-layout GLiFormer wrapper."""

    config_class = GLiFormerLayoutConfig
    model_class = GLiFormerLayoutModel
    data_collator_class = GLiFormerLayoutDataCollator


class GLiFormerOmni(BaseGLiFormer):
    """User-facing omni-modal GLiFormer wrapper."""

    config_class = GLiFormerOmniConfig
    model_class = GLiFormerOmniModel
    data_collator_class = GLiFormerOmniDataCollator


_GLIFORMER_MODEL_TO_WRAPPER = {
    GLiFormerTextModel: GLiFormerText,
    GLiFormerModel: GLiFormerText,
    GLiFormerVisionModel: GLiFormerVision,
    GLiFormerAudioModel: GLiFormerAudio,
    GLiFormerLayoutModel: GLiFormerLayout,
    GLiFormerOmniModel: GLiFormerOmni,
}

class GLiFormer(nn.Module, PyTorchModelHubMixin):
    """Factory class that instantiates the appropriate GLiFormer wrapper.

    Mirrors :class:`gliner.model.GLiNER`: the factory reads the config,
    resolves the concrete GLiFormer type, delegates construction/loading to that
    subclass, then replaces itself with the concrete instance.
    """

    def __init__(self, config: Union[str, Path, GLiFormerConfig, dict], **kwargs):
        super().__init__()
        config = self._coerce_config(config)
        gliformer_class = self._get_gliformer_class(config)
        new_instance = gliformer_class(config, **kwargs)
    ...
    def train_head_only_parameters(self) -> Dict[str, int]:
        ...
        Construction normally turns this factory into a concrete GLiFormer
        wrapper. Keeping the method on the public factory as well preserves the
        API for callers that invoke helpers through :class:`GLiFormer` directly.
        """

        return BaseGLiFormer.train_head_only_parameters(self)

    @staticmethod
    def _config_class_for_variant(config_dict: Dict[str, Any]):
        variant = config_dict.get("model_variant") or "text"
        try:
            return GLIFORMER_VARIANT_TO_CONFIG_CLASS[variant]
        except KeyError as exc:
            raise ValueError(
                "model_variant must be one of "
                f"{sorted(GLIFORMER_VARIANT_TO_CONFIG_CLASS)}, got {variant!r}."
            ) from exc

    @classmethod
    def _coerce_config_dict(cls, config_dict: Dict[str, Any]) -> GLiFormerConfig:
    ...
    @classmethod
    def _coerce_config(cls, config: Union[str, Path, GLiFormerConfig, dict]) -> GLiFormerConfig:
        ...
        if isinstance(config, GLiFormerConfig):
        ...
        raise TypeError(f"config must be a GLiFormerConfig object, path to config file, or dict. Got {type(config)}")

    @staticmethod
    def _get_gliformer_class(config: GLiFormerConfig):
        model_cls = resolve_gliformer_model_class(config)
        try:
            return _GLIFORMER_MODEL_TO_WRAPPER[model_cls]
        except KeyError as exc:
            raise ValueError(f"No GLiFormer wrapper registered for model class {model_cls}") from exc
    ...
    def from_pretrained(
        ...
        **model_kwargs,
        ...
        if model_dir is None:
            model_dir = BaseGLiFormer._download_model(
        ...
        config = BaseGLiFormer._load_config(
            config_file,
            max_len=max_length,
            max_width=max_width,
        ...
        gliformer_class = cls._get_gliformer_class(config)
        logger.info("Loading the following GLiFormer type: %s...", gliformer_class)
        loader_kwargs = BaseGLiFormer._supported_base_loader_kwargs(
            gliformer_class.from_pretrained,
            {
                "model_id": model_id,
                "model_dir": model_dir,
                "revision": revision,
        ...
        return gliformer_class.from_pretrained(**loader_kwargs)

    @classmethod
    def load_from_config(
        cls,
        config: Union[str, Path, GLiFormerConfig, dict],
        cache_dir: Optional[Union[str, Path]] = None,
        ...
        **model_kwargs,
        ...
        gliformer_class = cls._get_gliformer_class(config_instance)
        loader_kwargs = BaseGLiFormer._supported_base_loader_kwargs(
            gliformer_class.load_from_config,
            {
        ...
        return gliformer_class.load_from_config(**loader_kwargs)
