#!/usr/bin/env python3
"""Evaluate a GLiFormer joint-relation checkpoint on DocRED, CrossRE, and FewRel.
...
Example:
    python gliformer_eval/eval_relex.py \
...
DEFAULT_OUTPUT = Path("eval_results/gliformer_relex.json")
...
class RelationSample:
    ...
    # FewRel annotates one queried pair, not all relations in the sentence.
    query_pair: tuple[Entity, Entity] | None = None
...
def create_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate one GLiFormer joint-relation model on DocRED, CrossRE, and FewRel."
    )
    parser.add_argument(
        "--model",
        required=True,
    ...
    parser.add_argument(
        ...
        type=positive_int,
        default=1,
        help="GLiFormer inference batch size (default: 1).",
    )
    parser.add_argument(
        "--device",
        default="auto",
    ...
    parser.add_argument(
        ...
        default=0,
        help="Reject longer inputs rather than truncate gold evidence; 0 allows full text (default).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--schema-mode",
        choices=("dataset", "gold"),
        default="dataset",
        help=(
            "Candidate labels: all dataset types (default), or only each example's "
            "gold entity/relation types (oracle evaluation)."
        ),
    )
    parser.add_argument(
        "--schema-name",
...
def _download_url(url: str, destination: Path, *, local_files_only: bool) -> Path:
    ...
    request = Request(url, headers={"User-Agent": "GLiFormer relation evaluator"})
    ...
    *,
    ...
    for document_index, document in enumerate(documents):
        ...
        if relations:
            samples.append(
                RelationSample(
                    f"docred_{document_index}", text, tuple(relations),
                    tuple(sorted({entity.type for entity in entities})),
                )
            )
    ...
    with path.open(encoding="utf-8") as input_file:
        for line_number, line in enumerate(input_file):
            ...
            if relations:
                samples.append(
                    RelationSample(
                        f"crossre_{domain}_{line_number}", text, tuple(relations),
                        tuple(sorted({entity.type for entity in entities.values()})),
                    )
                )
    ...
    *,
    ...
    for relation_id, instances in data.items():
        ...
        for instance in instances:
            ...
            samples.append(RelationSample(f"fewrel_{sample_index}", text, (gold,), ("entity",),
                                          (gold.head, gold.tail)))
            sample_index += 1
...
def load_conll04(
    *, directory: Path, max_samples: int | None, seed: int,
) -> DatasetInfo:
    """Load the local IE-Bench CoNLL04 test split and declared label inventory."""
    with (directory / "labels.json").open(encoding="utf-8") as handle:
        relation_types = tuple(json.load(handle))
    with (directory / "test.json").open(encoding="utf-8") as handle:
    ...
    for index, row in enumerate(rows):
        ...
        for relation in row.get("relations", []):
            ...
            if label not in relation_types:
                raise ValueError(f"conll04 sample {index}: undeclared relation label {label!r}")
            head, tail = relation["head"], relation["tail"]
            relations.append(GoldRelation(
                Entity(head["name"], head["type"]),
                Entity(tail["name"], tail["type"]),
                label,
            ))
            sample_entity_types.update((head["type"], tail["type"]))
        entity_types.update(sample_entity_types)
        samples.append(RelationSample(
            f"conll04-{index}", row["sentence"], tuple(relations),
            tuple(sorted(sample_entity_types)),
        ))
    return DatasetInfo(
        name="conll04", samples=_sample(samples, max_samples, seed),
        entity_types=tuple(sorted(entity_types)), relation_types=relation_types,
    )


def load_datasets(args: argparse.Namespace) -> list[DatasetInfo]:
    loaders = {
        "conll04": lambda: load_conll04(
            directory=args.conll04_dir,
...
def deduplicate_gold(relations: Sequence[GoldRelation]) -> list[GoldRelation]:
    """Use normalized text-triple identity, preserving all known endpoint aliases."""
    unique = {}
    for relation in relations:
        key = (normalize(relation.head.text), normalize(relation.tail.text), normalize(relation.relation))
        previous = unique.get(key)
        if previous:
            relation = replace(previous,
                head=replace(previous.head, aliases=tuple(dict.fromkeys(
                    (*previous.head.aliases, relation.head.text, *relation.head.aliases)))),
                tail=replace(previous.tail, aliases=tuple(dict.fromkeys(
                    (*previous.tail.aliases, relation.tail.text, *relation.tail.aliases)))))
        unique[key] = relation
    return list(unique.values())


def deduplicate_predictions(relations: Sequence[PredictedRelation]) -> list[PredictedRelation]:
    unique = {}
    for relation in relations:
        key = (normalize(relation.head_text), normalize(relation.tail_text), normalize(relation.relation))
        if key not in unique or relation.score > unique[key].score:
            unique[key] = relation
    return list(unique.values())


def predictions_for_sample(sample: RelationSample, predictions: Sequence[PredictedRelation]) -> list[PredictedRelation]:
    """Project extraction baselines onto FewRel's queried pair without its label."""
    if sample.query_pair is None:
        return list(predictions)
    head, tail = sample.query_pair
    return [prediction for prediction in predictions
            if prediction.relation.startswith("__invalid_item__") or (
                _entity_matches(head, prediction.head_text) and _entity_matches(tail, prediction.tail_text))]


def validate_text_limit(samples: Sequence[RelationSample], max_text_chars: int) -> None:
    """Never score full-document gold against a silently clipped input."""
    if max_text_chars < 0:
        raise ValueError("--max-text-chars must be nonnegative")
    for sample in samples:
        if max_text_chars and len(sample.text) > max_text_chars:
            raise ValueError(f"{sample.id}: input has {len(sample.text)} characters, exceeding "
                             f"--max-text-chars {max_text_chars}; increase the limit or use 0. "
                             "Input truncation would hide gold evidence.")
...
def _match_relations(
    ...
    gold_relations = deduplicate_gold(gold_relations)
    predicted_relations = deduplicate_predictions(predicted_relations)
...
def compute_metrics(
    ...
    for sample, sample_predictions in zip(samples, predictions, strict=True):
        matched, false_positives, false_negatives = _match_relations(
            sample.relations, predictions_for_sample(sample, sample_predictions)
        )
        ...
        cardinality = _cardinality(deduplicate_gold(sample.relations))
...
def convert_predictions(
    ...
    *,
    ...
    """Normalize native GLiFormer triples and prune labels per entity pair."""
...
def load_model(
    ...
    *,
    ...
    from gliformer import GLiFormer  # noqa: PLC0415 - defer heavyweight model imports
    ...
    if getattr(model.config, "joint_relex_config", None) is None:
        raise ValueError(f"GLiFormer checkpoint {model_id!r} does not enable a joint_relex head")
    model.to(device)
    model.eval()
    return model
...
def evaluate_dataset(
    ...
    *,
    ...
    max_relations_per_pair: int,
    schema_name: str,
    schema_mode: str = "dataset",
) -> tuple[list[list[PredictedRelation]], float]:
    """Run batched joint-relation inference for one prepared dataset."""
    if schema_mode not in ("dataset", "gold"):
        raise ValueError(f"Unknown schema mode: {schema_mode}")
    validate_text_limit(dataset.samples, max_text_chars)
    # inference() accepts one shared schema per call. Group identical schemas
    # so batching never exposes labels from another example in gold mode.
    groups: dict[tuple[tuple[str, ...], tuple[str, ...]], list[int]] = defaultdict(list)
    for index, sample in enumerate(dataset.samples):
        if schema_mode == "gold":
            entity_types = tuple(sorted(
                set(sample.entity_types)
                | {entity.type for relation in sample.relations
                   for entity in (relation.head, relation.tail)}
            ))
            relation_types = tuple(sorted({relation.relation for relation in sample.relations}))
        else:
            entity_types, relation_types = dataset.entity_types, dataset.relation_types
        groups[(entity_types, relation_types)].append(index)
    planned_batches = [
        (indices, entity_types, relation_types)
        for (entity_types, relation_types), group in groups.items()
        for indices in _batched(group, batch_size)
    ]
    all_predictions: list[list[PredictedRelation]] = [[] for _ in dataset.samples]
    batches = len(planned_batches)
    progress_interval = max(1, batches // 100)
    started = time.perf_counter()
    for batch_index, (indices, entity_types, relation_types) in enumerate(planned_batches, start=1):
        batch = [dataset.samples[index] for index in indices]
        schema = {
            schema_name: {"entities": list(entity_types), "relations": list(relation_types)}
        }
        ...
        if not isinstance(raw_batch, list) or len(raw_batch) != len(batch):
            ...
            raise RuntimeError(
                f"GLiFormer returned invalid joint_relex output for {dataset.name}: "
                f"expected {len(batch)} samples, got {actual}"
            )
        for index, raw_sample in zip(indices, raw_batch, strict=True):
            all_predictions[index] = convert_predictions(
                raw_sample,
                relation_types,
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "settings": {
            ...
            "fewrel_protocol": "extract_then_score_queried_pair",
            "duplicate_policy": "normalized_text_triples_on_both_sides",
