...
class BaseGLiFormerProcessor(TextProcessingMixin, BaseProcessor):
...
class GLiFormerTextProcessor(BaseGLiFormerProcessor):
    ...
    def _canonicalize_structuring_tokens(self, item):
        ...
        if active_processors and all(
            ...
            return
        if item.get("_gliformer_tokens_are_authoritative"):
            return
        if any(
    ...
    def _build_label_batch_list(self, batch):
        ...
        for i in range(len(batch["tokens"])):
            item = {
                ...
                "_gliformer_extraction_spans_resolved": True,
    ...
    def tokenize_and_prepare_labels(self, batch, prepare_labels=True, *args, **kwargs):
        ...
        if prepare_labels:
            ...
            if open_rel_result is not None:
                tokenized_input.update(open_rel_result)
...
class GLiFormerVisionProcessor(VisionProcessingMixin, BaseGLiFormerProcessor):
...
class GLiFormerAudioProcessor(AudioProcessingMixin, BaseGLiFormerProcessor):
...
class GLiFormerLayoutProcessor(LayoutProcessingMixin, VisionProcessingMixin, GLiFormerTextProcessor):
    ...
    def collate_raw_batch(self, batch_list, **kwargs):
        ...
        batch = GLiFormerTextProcessor.collate_raw_batch(self, batch_list, **kwargs)
        batch.update(self._prepare_layout_vision_batch(batch_list))
        batch.update(layout_fields)
        return batch

    def tokenize_and_prepare_labels(self, batch, prepare_labels=True, *args, **kwargs):
        tokenized_input = GLiFormerTextProcessor.tokenize_and_prepare_labels(
...
class GLiFormerOmniProcessor(
    ...
    def _build_label_batch_list(self, batch):
        batch_list = GLiFormerTextProcessor._build_label_batch_list(self, batch)
        for i, item in enumerate(batch_list):
            for field in (
...
def resolve_gliformer_processor_class(config):
    ...
    if variant == "layout":
        return GLiFormerLayoutProcessor
    if variant == "vision":
        return GLiFormerVisionProcessor
    if variant == "audio":
        return GLiFormerAudioProcessor
    if variant == "omni":
        return GLiFormerOmniProcessor
    if variant == "text":
        return GLiFormerTextProcessor
    raise ValueError(f"Unknown GLiFormer model_variant: {getattr(config, 'model_variant', None)!r}")


class GLiFormerProcessor(GLiFormerOmniProcessor):
    """Backward-compatible processor class.

    Existing imports keep working and retain all modality capabilities. New
    construction paths should use :func:`resolve_gliformer_processor_class`.
    """
    pass
