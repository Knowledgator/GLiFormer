"""Entity-first set-prediction open relation extraction."""
...
class OpenRelexHead(NERHead):
    ...
    * ``extra["assignment_logits"]`` has shape ``(BN, N, E, 2)`` and selects
    ...
    def __init__(self, config, hidden_size, dropout, shared_layers=None):
        task_cfg = config.open_relex_config
        ...
        self.entity_loss_coef = float(task_cfg.entity_loss_coef)
        self.assignment_loss_coef = float(task_cfg.assignment_loss_coef)
        ...
        self._relation_anchor_capacity = int(
            getattr(
                self.relation_anchor_layer,
                "num_slots",
                getattr(groups_layer, "max_count", task_cfg.max_count),
            )
        )
        ...
        if hasattr(self, "relation_anchor_refine"):
            self.relation_anchor_refine_positions = (
                self._build_refinement_positions(task_cfg, hidden_size)
            )
            (
        ...
        self.use_anchor_objectness = bool(task_cfg.anchor_objectness)
    ...
    def _sanitize_spans(span_idx, span_mask, sequence_length, word_mask=None):
        if span_idx.dim() != 3 or span_idx.shape[-1] != 2:
            raise ValueError("open relation spans must have shape (BN, E, 2)")
    ...
    def _coalesce_spans(span_idx, span_mask, assignment_labels=None):
        ...
        if assignment_labels.dim() != 4:
            raise ValueError(
                "open_rel_assignment_labels must have shape (BN, G, E, 2)"
            )
        if assignment_labels.shape[0] != span_idx.shape[0] or (
            assignment_labels.shape[2] != span_idx.shape[1]
        ):
            raise ValueError(
                "open relation endpoint labels must share the span entity axis"
            )
    ...
    def _relation_anchors(
        ...
        if not self._relation_uses_fixed_slots:
            ...
            # and inference. Passing open_rel_count here would leak the
    ...
    def _assignment_loss(
        ...
        if relation_labels.dim() != 3:
            raise ValueError(
                "open_rel_labels must have shape (BN, G, R)"
            )
        ...
        if assignment_labels.shape[:2] != relation_labels.shape[:2]:
            raise ValueError("open relation targets must share (BN, G)")
        if relation_labels.shape[0] != relation_logits.shape[0] or (
            assignment_labels.shape[0] != assignment_logits.shape[0]
        ):
            raise ValueError("open relation targets and logits must share BN")
        ...
        if overflow.any() and not self._relation_capacity_warning_emitted:
            ...
            warnings.warn(
                "Open relation gold count exceeds the active "
    ...
    def forward(
        ...
        **batch,
        ...
        entity_labels = batch.get("open_rel_entity_labels")
        span_idx = batch.get("open_rel_span_idx")
        span_mask = batch.get("open_rel_span_mask")
        relation_labels = batch.get("open_rel_labels")
        assignment_labels = batch.get("open_rel_assignment_labels")
