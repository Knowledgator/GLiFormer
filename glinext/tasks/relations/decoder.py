"""Relations task decoder — post-processing logits into relation triples."""
...
class RelationsDecoder:
    ...
    def decode(self, model_output, classes_mapping=None, threshold=None, **kwargs) -> List[List[dict]]:
        ...
        Args:
            model_output: GLiNExTOutput with rel_logits, rel_idx, rel_mask.
            classes_mapping: BatchClassesMapping for label resolution.
            threshold: Override detection threshold.

        Returns:
            List of lists of triple dicts: [{head_id, tail_id, rel_class_id, score}, ...] per group.
        """
        if model_output.rel_logits is None or model_output.rel_idx is None:
            return []

        threshold = threshold or self.threshold
        probs = torch.sigmoid(model_output.rel_logits)
        pair_idx = model_output.rel_idx
        pair_mask = model_output.rel_mask
